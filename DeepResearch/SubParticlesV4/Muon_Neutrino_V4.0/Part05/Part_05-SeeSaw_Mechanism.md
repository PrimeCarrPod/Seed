...
For $\nu_R^{(1)}$ (octo, 3.5 TeV):
$$C_{0.1, \text{octo}}(0) \approx 0.05 \times 100 = 5$$

$$\mathcal{M}_\nu^{(1)} \sim \frac{5^2}{3.5 \times 10^6 \text{ MeV}} \approx 7 \text{ meV}$$
...
Rank-2 matrix $\to$ two non-zero masses, one massless neutrino.
