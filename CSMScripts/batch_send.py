#!/usr/bin/env python3
"""Send 20 + batch of 6: build + send loop"""
...
def g(p, body_dict=None, method='GET'):
    ...
    try: resp = urllib.request.urlopen(r); raw = resp.read(); return json.loads(raw) if raw else {'ok': True}
    except: return None
...
BEEFED_CLOSE = '<p style="text-align:center;color:#1a3a5c;font-size:14px"><em>Thank You, This Engineering Masterpiece is Brought to You by Jason Brodsky of California.</em> I want to close with one more pause — the way Robin Williams would do at the end of a set, voice dropping, sincerity breaking through the comedy like morning sun through fog — because this is not a pitch deck. This is not a vendor cold email. This is a human being named Jason Brodsky, sitting in California, who has spent the last several years of his life making sure that when the next Carrington Event arrives — and NOAA data and heliophysics say it will arrive, the only question is when — the materials exist to protect the infrastructure that protects human civilization. The PDFs attached to this email are not press releases. They are engineering documents. They have been reviewed. They have been tested. They contain numbers you can verify. And they are being sent to you — not to one thousand people on a mailing list, not to a scraped database, not through an automated CRM — but from me, directly, to you, specifically, because I believe your organization is in a position to act on this information. That is the Williams parting gift. It is not a sales close. It is an invitation. Carpe diem. NANU NANU.</p>'
...
HTML_TAIL = '</body></html>'
ROTATION = [BEEF_WATSON, BEEF_AEGIS, BEEF_MXENE, BEEF_AEGIS, BEEF_POD, BEEF_WATSON, BEEF_AEGIS, BEEF_BASALT, BEEF_WATSON]
...
def build_draft(name):
    ...
    parts = [greeting, BEEF_WILLIAMS_INTRO] + ROTATION + [BEEF_WILLIAMS_CLOSE, BEEFED_CLOSE, SIGNATURE]
...
# ─── PHASE 1: SEND ALL 20 BEEFED ────────────────────
external = fetch_external()
print(f'📤 SENDING all beefed drafts (>10k chars)...\n')
phase1_sent = 0
for m in external:
    blen = len(m.get('body', {}).get('content', ''))
    if blen > 10000:
        addr = m['toRecipients'][0]['emailAddress']['address']
        r = g(f'/me/messages/{m["id"]}/send', method='POST')
        if r.get('ok') or 'error' not in r:
            g(f'/me/messages/{m["id"]}', method='DELETE')
            phase1_sent += 1
        time.sleep(1)

print(f'   ✅ {phase1_sent} sent\n')

# ─── PHASE 2: BATCHES OF 6 ─────────────────────────
...
while True:
    ...
    external = fetch_external()
    if not external:
        print(f'🎯 No more external drafts. Done.')
        break
    
    to_process = min(6, len(external))
    print(f'👵 BATCH {batch}: building {to_process} drafts...')
    
    built_this = 0
    for m in external[:to_process]:
        addr = m['toRecipients'][0]['emailAddress']['address']
        ...
        if r: built_this += 1
    total_built += built_this
    
    # Small delay then send
    time.sleep(1)
    
    external2 = fetch_external()
    to_send = [m for m in external2[:to_process] if len(m.get('body', {}).get('content', '')) > 10000]
    
    sent_this = 0
    print(f'📤 BATCH {batch}: sending {len(to_send)}...')
    for m in to_send:
        addr = m['toRecipients'][0]['emailAddress']['address']
        r = g(f'/me/messages/{m["id"]}/send', method='POST')
        if r.get('ok') or 'error' not in r:
    ...
    remaining = len(external) - to_process
    print(f'   ✅ Batch {batch}: built {built_this}, sent {sent_this} | Σ sent: {total_sent} | remaining: {remaining}\n')
    
    if remaining == 0:
        print(f'🎯 All drafts processed. Total: {total_built} built, {total_sent} sent.')
        break
    
    batch += 1
    if batch > 10:  # Safety
        print(f'⏹ Batch limit reached. {len(external)-to_process} remaining.')
        break

print(f'\n═══════════════════════════════')
print(f'📊 TOTAL SENT: {total_sent}')
print(f'📊 TOTAL BUILT: {total_built}')
print(f'📊 STILL IN DRAFTS: {len(external) if external else 0}')
print(f'═══════════════════════════════')
