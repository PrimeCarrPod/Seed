#!/usr/bin/env python3
"""
THE OLD LADY — BEEF CHECKER v2.0
CSMSOPP000004 · Starlight Courier Heuristic

DRAFT FORMAT (POST-OLD-LADY BEEF):
  P1:  Personalized greeting — "[Name], my name is Jason Brodsky. Carrington Storm Motors.
       We build physical hardening against Carrington-level CMEs. I have recruited
       Robin Williams Heuristics to help explain things, oh please do enjoy."
  P2:  Williams beefy overview — how we help and teach (Watson-style)
  P3-P11: 9 beefy paragraphs spoken through heuristics of Aegis agents (unnamed)
         Rotation: Aegis → Waton → MXene → Aegis → Pod → Watson → Aegis → Basalt → Watson
  P12: Williams beefy summary paragraph
  P13: Close — "Thank You, This Engineering Masterpiece is Brought to You by Jason Brodsky of California"
  P14: Signature block
"""
...
BEEF_ROTATION = [BEEF_WATSON, BEEF_AEGIS, BEEF_MXENE, BEEF_AEGIS, BEEF_POD,
                 BEEF_WATSON, BEEF_AEGIS, BEEF_BASALT, BEEF_WATSON]
assert len(BEEF_ROTATION) == 9

def extract_name(m):
    """Extract recipient first name from the draft's toRecipient or original body."""
    tos = m.get('toRecipients', [])
    if tos:
        full_name = tos[0]['emailAddress'].get('name', '')
        if full_name:
            return full_name.split('—')[0].split('–')[0].strip()
    body = m.get('body', {}).get('content', '')
    match = re.search(r'<p[^>]*>\s*(\w+)[\s—–]', body)
    if match: return match.group(1)
    return 'Friend'

def build_draft(name):
    greeting = f'<p style=\"font-family:Georgia,serif;font-size:15px;color:#222;line-height:1.5\">{name}, my name is Jason Brodsky. Carrington Storm Motors. We build physical hardening against Carrington-level CMEs. I have recruited Robin Williams Heuristics to help explain things, oh please do enjoy.</p>'
    parts = [greeting, BEEF_WILLIAMS_INTRO] + BEEF_ROTATION + [BEEF_WILLIAMS_CLOSE, CLOSE, SIGNATURE]
...
print(f'👵 OLD LADY v2.0 — Rebuilding {len(external)} drafts with proper format\n')
...
for i, m in enumerate(external[:5]):
    ...
    result = g(f'/me/messages/{mid}', {'body': {'contentType': 'HTML', 'content': new_body}}, method='PATCH')
    status = '✅' if result else '❌'
    print(f'  [{i+1}/5] {status} {to_addr:<42} | {name:<25} | {len(new_body)} chars | {subj[:55]}')
    time.sleep(0.8)
...
print(f'\n{"="*65}')
print('VERIFICATION:')
check = g(f'/me/mailFolders/drafts/messages?{q}')
for m in check.get('value', []):
    tos = m.get('toRecipients', [])
    if not tos: continue
    addr = tos[0]['emailAddress']['address'].lower()
    if any(w in addr for w in ['hotmail.com', 'outlook.com', 'aegisc.space']): continue
    blen = len(m.get('body', {}).get('content', ''))
    pc = len(re.findall(r'<p[ >]', m.get('body', {}).get('content', '')))
    tag = '👵 OLD LADY v2' if blen > 8000 else ''
    print(f'  {addr:<45} {blen:>5}c {pc}p {tag}')
