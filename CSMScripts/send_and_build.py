#!/usr/bin/env python3
"""Send all beefed drafts + build 20 more with Old Lady v2"""
...
def build_draft(name):
    ...
    parts = [greeting, BEEF_WILLIAMS_INTRO] + ROTATION + [BEEF_WILLIAMS_CLOSE, CLOSE, SIGNATURE]
    return HTML_HEAD + ''.join(parts) + HTML_TAIL

# ─── FETCH ALL EXTERNAL DRAFTS ──────────────────
...
# ─── SEND: find beefed drafts (>10k chars) ─────
print('📤 SENDING beefed drafts (>10k chars):\n')
sent_count = 0
for m in external:
    ...
    if blen > 10000:
        ...
        if r.get('ok') or 'error' not in r:
            print(f'  ✅ {addr:<42} {blen}c')
            g(f'/me/messages/{m["id"]}', method='DELETE')
            sent_count += 1
        else:
            print(f'  ❌ {addr:<42} FAILED')
        time.sleep(1.2)

# Re-fetch after sends
time.sleep(2)
q2 = urllib.parse.quote('$top=200&$select=id,subject,toRecipients', safe='=&$')
drafts2 = g(f'/me/mailFolders/drafts/messages?{q2}')
remaining = []
for m in drafts2.get('value', []):
    tos = m.get('toRecipients', [])
    if not tos: continue
    addr = tos[0]['emailAddress']['address'].lower()
    if any(w in addr for w in ['hotmail.com', 'outlook.com', 'aegisc.space']): continue
...
# ─── BUILD 20 MORE ─────────────────────────────
to_build = min(20, len(remaining))
print(f'\n👵 BUILDING {to_build} more drafts:\n')
built = 0
for i, m in enumerate(remaining[:to_build]):
    addr = m['toRecipients'][0]['emailAddress']['address']
    name = m['toRecipients'][0]['emailAddress'].get('name', '')
    name = name.split('—')[0].split('–')[0].strip() or 'Friend'
    new_body = build_draft(name)
    r = g(f'/me/messages/{m["id"]}', {'body': {'contentType': 'HTML', 'content': new_body}}, method='PATCH')
    status = '✅' if r else '❌'
    if r: built += 1
    print(f'  [{i+1}/{to_build}] {status} {addr:<45} {len(new_body)}c')
    time.sleep(0.7)

print(f'\n═══════════════════════════════')
print(f'📊 SENT: {sent_count} drafts')
print(f'📊 BUILT: {built}/{to_build} drafts')
print(f'📊 REMAINING: {len(remaining)-to_build} external drafts left')
print(f'═══════════════════════════════')
